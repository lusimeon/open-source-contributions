...
<template>
  <div class="flex items-center gap-2 sm:gap-4">
    <a
      ...
      target="_blank"
      relative
      :class="['size-10 sm:size-12 shrink-0 border border-gray-100 overflow-hidden shadow-sm', data.type === 'Organization' ? 'rounded-lg' : 'rounded-full']"
    >
    ...
    <div class="flex-1 flex justify-between gap-2 lg:gap-4 min-w-0">
      <div class="flex flex-col min-w-0 gap-0.5 sm:gap-1">
        ...
        <div class="flex gap-2 items-bottom">
          <a :href="`https://github.com/${data.repo}`" target="_blank" class="text-sm sm:text-base inline-flex gap-1 hover:text-black truncate">
            <span class="opacity-75">{{ data.repo.split('/')[0] }}</span>
            <span class="opacity-50">/</span>
            <span class="truncate">{{ data.repo.split('/')[1] }}</span>
          </a>
          <a :href="`https://github.com/${data.repo}`" target="_blank" class="items-center hidden sm:inline-flex gap-0.5 hover:text-black truncate">
            <UIcon name="i-ph-star" class="size-3 shrink-0" />
            <span class="text-xs text-gray-500">{{ formatStars(data.stars) }}</span>
          </a>
        </div>
      </div>
